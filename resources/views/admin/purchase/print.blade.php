...
<table style="border: none; width: 100%;">
    ...
    @foreach ($purchase->purchaseDetail as $item)
        ...
        <tr>
            ...
            <td style="width: 15%; text-align: center;">{{ $item->healthy_amount_of_chilies + $item->number_of_damaged_chilies }}</td>
            <td style="width: 15%; text-align: center;">{{ 'Rp ' . number_format($item->chilliPrice->price, 2, ',', '.') }}</td>
            <td style="width: 20%; text-align: right;">{{ 'Rp ' . number_format($item->healthy_amount_of_chilies * $item->chilliPrice->price, 2, ',', '.') }}</td>
        </tr>
    @endforeach
    @php
